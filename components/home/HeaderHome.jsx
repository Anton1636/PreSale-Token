...
const HeaderHome = ({ isDarkMode, toggleDarkMode }) => {
	...
	const megaMenus = {
				...
				},
				{
					...
					links: [
						...
						},
						{
							...
							label: 'Explorer',
							href: `${EXPLORER_TOKEN_URL}${MTK_ADDRESS}`,
						},
						{
		...
		resources: {
					...
					links: [
						{
							...
							href: `${EXPLORER_ADDRESS_URL}${MTK_ADDRESS}`,
						},
						{
			...
			featuredBox: {
				...
				description:
					'Access developer resources and start building on the Protocol.',
		...
		<>
			...
			<header
				className={`w-full transition-all duration-500 ease-out fixed top-0 left-0 z-50 shadow-lg animate-slowSlideDown border-b ${
					isDarkMode
						? 'bg-[#oE0B12]/95 backdrop-blur-md border-gray-800/50'
			...
			>
					...
					<div className='relative py-3 overflow-hidden whitespace-nowrap'>
						...
						<div className='animate-marquee inline-block whitespace-nowrap text-white relative z-10'>
							<span className='mx-4 text-sm md:text-base'>
								{TOKEN_NAME} ({TOKEN_SYMBOL}) 🚀 Pre-Sale is LIVE! Join the
								future — claim your tokens and get exclusive early access to
				...
				<div className='container mx-auto px-4 py-4 flex justify-between items-center'>
					<div className='flex items-center'>
						<Link href='/' className='flex items-center group'>
							...
							<span className='text-3xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-50 to-pink-600 animate-gradient-x'>
								{TOKEN_NAME}
							</span>
						</Link>
					</div>
					...
					<div className='hidden lg:flex items-center space-x-4'>
						<button
							...
							className={`p-2 rounded-full transition-all duration-300 ${
								isDarkMode
									? 'bg-gradient-to-r from-fuchsia-500 to-pink-600 hover:from-fuchsia-600 hover:to-purple-700 text-white'
									: 'bg-gradient-to-r from-fuchsia-500 to-pink-600 hover:from-fuchsia-600 hover:to-purple-700 text-white'
							}`}
							aria-label={
						...
						<a href='/dashboard' className='group'>
							<div className='w-10 h-10 bg-gradient-to-r from-fuchsia-500 to-pink-600 hover:from-fuchsia-600 hover:to-purple-700 rounded-full flex items-center justify-center transition-all duration-300 shadow-md hover:shadow-lg hover:scale-105'>
							...
							<div className='container mx-auto py-8 px-4'>
								<div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6'>
												...
												{column.links.map((link, linkIdx) => (
													<li key={linkIdx}>
														<Link
															href={link.href}
															target='_blank'
															className={`flex items-center space-x-2 py-1 transition duration-200
															${
																isDarkMode
																	? 'text-gray-300 hover:text-fuchsia-500'
																	: 'text-gray-700 hover:text-teal-600'
									...
									<div className={`rounded-xl p-6 ${menu.featuredBox.bgClass}`}>
										<h3
											className={`text-xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-fuchsia-500 to-pink-600`}
										>
				...
				${
					isDarkMode
						? 'bg-[#14101A]/95 border-r border-gray-800/50'
				...
				>
					<div className='h-full overflow-auto'>
						<div
							className={`p- border-b ${
								isDarkMode ? 'border-gray-900/50' : 'border-gray-200/50'
								...
								<Link
									...
									className={`flex items-center space-x-2 py-4 border-b ${
										isDarkMode
											...
											: 'border-gray-800/50 text-gray-700 hover:text-fuchsia-600'
									}`}
									onClick={() => setIsOpen(false)}
								>
									<FiCompass size={18} />
